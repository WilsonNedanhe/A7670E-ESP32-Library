...
// Run with: pio test -e esp32dev -f test_uart --upload-port <port>
//
// This suite is NOT run in CI (see .github/workflows/build.yml, which
// only compiles the library and examples). UART talks to real modem
// hardware, so it can't be faked the way test_at's sendSMS test is —
// this is PWRKEY/RESET sequencing and readLine() against a real A7670E on Serial2.
//
...
void test_placeholder(void) {
    TEST_IGNORE_MESSAGE("UART hardware test not yet implemented — see file header.");
}
